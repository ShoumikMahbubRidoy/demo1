...
class CANUtils:
    ...
    def decode_motor_feedback(msg: can.Message) -> Tuple[int, float, float, float]:
        """
        Decode motor feedback from CAN message
        
        Args:
            msg: Received CAN message
            
        Returns:
            Tuple of (motor_id, position, velocity, torque)
        """
        ...
        # Decode according to AK60-6 feedback format
        ...
        velocity = CANUtils.uint_to_float(vel_int, -30.0, 30.0, 12)
        torque = CANUtils.uint_to_float(torque_int, -12.0, 12.0, 12)
